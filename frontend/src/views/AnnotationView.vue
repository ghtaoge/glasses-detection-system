...
<template>
  <section class="annotation-page">
    ...
    <div v-if="current" class="annotation-layout">
      <aside class="image-queue"><button v-for="(image,i) in images" :key="image.id" :class="{active:i===index}" @click="index=i;sync()"><img :src="image.url" alt="" /><span>{{ image.original_name }}</span><small>{{ image.review_state==='reviewed'?'已复核':'待复核' }}</small></button></aside>
      <div class="canvas-column"><div class="canvas-toolbar"><button class="icon-button" title="上一张" aria-label="上一张" :disabled="index===0" @click="move(-1)"><ChevronLeft :size="18" /></button><span>{{ index+1 }} / {{ images.length }}</span><button class="icon-button" title="下一张" aria-label="下一张" :disabled="index===images.length-1" @click="move(1)"><ChevronRight :size="18" /></button></div><AnnotationCanvas :image="current.url" :image-width="current.width" :image-height="current.height" :annotations="annotations" :active-class="activeClass" @create="add" @select="selected=$event" /></div>
